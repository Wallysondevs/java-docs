# A ferramenta de linha de comando JFR

[Início](<#/>) > [Tutoriais](<#/doc/tutorials/learn>) > [ JDK Flight Recorder ](<#/doc/tutorials/jvm/jfr>) > A ferramenta de linha de comando JFR
...
**Anterior na Série:** [Configurando o JDK Flight Recorder](<#/doc/tutorials/jvm/jfr/configure>)
...
Como truncar conteúdo que excede o espaço em uma célula de tabela. O modo pode ser 'beginning' (início) ou 'end' (fim). O valor padrão é 'end'.
...
Uma lista de arquivos .jfc separados por vírgulas nos quais a nova configuração é baseada. Se nenhum arquivo for especificado, o arquivo padrão no JDK é usado (default.jfc). Se 'none' for especificado, a nova configuração começa vazia.
...
`<output-file>` O arquivo de saída para gravar eventos filtrados. Se nenhum arquivo for especificado, ele será gravado no mesmo caminho do arquivo de entrada, mas com "-scrubbed" anexado ao nome do arquivo.
...
Use `jfr disassemble` para decompor um arquivo de gravação de voo em seus pedaços de arquivo de chunk. A sintaxe é:
...
O local para gravar o arquivo desmontado, por padrão o diretório atual
...
Apresentando Jfr Sinopse Descrição Subcomandos Subcomandos de Versão e Ajuda
